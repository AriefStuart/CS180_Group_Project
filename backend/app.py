...
def get_posts_for_user_and_friends(user_id):
    ...
    posts = Posts.query.filter(
        (Posts.user_id == user.id) | (Posts.user_id.in_([friend.id for friend in friends]))
    ).all()
...
def get_users_excluding_me():
    current_user_id = request.args.get('user_id')  # Pass the current user's ID as a query parameter
    ...
    for user in users:
        if user.id != current_user.id:  # Exclude the current user
            user_list.append({
                ...
                "isFriend": user in current_user.friends  # Check if the user is a friend
            })
...
def get_friends(user_id):
    ...
    friends = [{"id": friend.id, "username": friend.username} for friend in user.friends]
...
def toggle_like(post_id, user_id):
    ...
    if like:
        ...
        post.like_count -= 1
        db.session.commit()
        return jsonify({"post_id": post_id, "like_count": post.like_count, "liked": False})
    else:
        # Like the post
        ...
        post.like_count += 1
        db.session.commit()
        return jsonify({"post_id": post_id, "like_count": post.like_count, "liked": True})
